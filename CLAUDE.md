...
practice. The server exposes 107 tools whose names start with `gephi_`
...
Install the plugin rather than registering the bare server; it bundles the
server, the skill, the slash commands (`/import-and-explore`, `/explore`,
`/analyze-network`, `/community-detection`, `/centrality`, `/counterfactual`,
`/verify-claim`, `/text-network`, `/teach`, `/visualize`, `/export-map`), and
the subagents (`network-analyst`, `claim-verifier`, `layout-iterator`,
